...
import { composeBackdrop, GLASS, keyOf, ROOM_HEIGHT, ROOM_WIDTH, skylineAt, type BackdropKey } from "./compose";
...
export class Backdrop {
  ...
  render(
    ...
    if (celestial.body === "moon") this.renderStars(ctx, seed, clock, reducedMotion);
    this.renderCelestial(ctx, celestial, key.weather, key.themeId);
    this.renderClouds(ctx, key.weather, seed, clock);
    if (key.weather === "rain") this.renderRain(ctx, seed, clock);
    if (key.weather === "snow") this.renderSnow(ctx, seed, clock);
  ...
  private blitArchitecture(ctx: CanvasRenderingContext2D, key: BackdropKey): void {
    ...
    if (wanted !== this.cacheKey || !this.canvas) {
      ...
      image.data.set(composeBackdrop(key));
  ...
  private renderStars(ctx: CanvasRenderingContext2D, seed: number, nowMs: number, reducedMotion: boolean): void {
    for (let index = 0; index < STAR_COUNT; index++) {
      const x = GLASS.x + Math.floor(hash(seed + index * 31) * GLASS.w);
      const y = GLASS.y + Math.floor(hash(seed + index * 57) * (GLASS.h * 0.62));
  ...
  /** The sun or the moon, riding its arc across the pane. */
  private renderCelestial(ctx: CanvasRenderingContext2D, celestial: Celestial, weather: Weather, themeId: string): void {
    // Overcast skies swallow the disc; a hint survives so the hour still reads.
    const veiled = weather === "rain" || weather === "cloudy";
    const x = Math.round(GLASS.x + 4 + celestial.t * (GLASS.w - 8));
    const y = Math.round(GLASS.y + GLASS.h - 6 - celestial.height * (GLASS.h - 14));
    // The sun rises from behind the town and sets behind it: clip the disc to
    // the sky above whatever stands in its column.
    const horizon = GLASS.y + GLASS.h - skylineAt(themeFor(themeId), x - GLASS.x);
    ctx.save();
    ctx.beginPath();
    ctx.rect(GLASS.x, GLASS.y, GLASS.w, Math.max(0, horizon - GLASS.y));
    ctx.clip();
  ...
  private renderClouds(ctx: CanvasRenderingContext2D, weather: Weather, seed: number, nowMs: number): void {
    const tint = CLOUD_TINT[weather];
    const span = GLASS.w + 24;
    for (let index = 0; index < tint.count; index++) {
      const drift = (nowMs / (CLOUD_DRIFT_MS * (0.7 + hash(seed + index * 13) * 0.6))) % 1;
      const x = Math.round(GLASS.x - 12 + ((drift + hash(seed + index * 97)) % 1) * span);
      const y = GLASS.y + 3 + Math.floor(hash(seed + index * 41) * (GLASS.h * 0.4));
  ...
  private renderRain(ctx: CanvasRenderingContext2D, seed: number, nowMs: number): void {
    ...
    for (let index = 0; index < RAIN_DROPS; index++) {
      ...
      const x = GLASS.x + Math.floor(hash(seed + index * 11) * GLASS.w) - Math.floor(progress * 4);
      const y = GLASS.y + Math.floor(progress * GLASS.h);
    ...
    for (let index = 0; index < PANE_DROPS; index++) {
      ...
      const x = GLASS.x + 6 + Math.floor(hash(seed + index * 53) * (GLASS.w - 12));
      const y = GLASS.y + Math.floor(progress * GLASS.h);
      ctx.fillRect(x, y, 1, 2);
    }
  }

  private renderSnow(ctx: CanvasRenderingContext2D, seed: number, nowMs: number): void {
    ...
    for (let index = 0; index < SNOW_FLAKES; index++) {
      ...
      const x = Math.round(GLASS.x + hash(seed + index * 17) * GLASS.w + sway);
      const y = GLASS.y + Math.floor(progress * GLASS.h);
      ctx.fillRect(x, y, 1, 1);
    }
  }
