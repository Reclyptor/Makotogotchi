...
| `POST` | `/api/care` | `{ action, itemId? }` → performs a care action. `200` / `409` (illegal or cooling down) / `429` (rate limited). A `200` carries `itemKept: true` when a consumable was handed back because the action moved nothing (§13.2). |
